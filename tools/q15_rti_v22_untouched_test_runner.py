"""Crash-safe one-shot scoring of V22's sealed untouched test.
...
RESERVED_STATUS = "V22_UNTOUCHED_TEST_LABEL_ACCESS_RESERVED"
PASS_STATUS = "V22_HISTORICAL_GATES_PASSED_MANUAL_PAPER_CONSIDERATION_ONLY"
REJECT_STATUS = "V22_UNTOUCHED_TEST_GATE_FAILED_NO_PAPER_CHALLENGER"
...
def _validate_sealed(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if supplied != _canonical_sha256(output):
        raise ValueError("v22_untouched_test_state_sha256_invalid")
...
def _read_sealed(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("v22_untouched_test_state_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ValueError("v22_untouched_test_state_root_not_object")
...
def _now_iso(value: str | None) -> str:
    ...
    try:
        ...
        raise ValueError("v22_untouched_test_timestamp_invalid") from exc
    if parsed.tzinfo is None:
        raise ValueError("v22_untouched_test_timestamp_invalid")
...
def _load_passing_pretest(
    ...
    if not reservation_path.exists():
        raise ValueError("v22_untouched_test_pretest_reservation_missing")
    ...
    if not result_path.exists():
        raise ValueError("v22_untouched_test_pretest_ambiguous")
    ...
    if (
        ...
        raise ValueError("v22_untouched_test_pretest_gate_not_passed")
    try:
        bundle = joblib.load(artifact_path)
    except Exception as exc:
        raise ValueError("v22_untouched_test_model_bundle_unreadable") from exc
    if not isinstance(bundle, Mapping):
        raise ValueError("v22_untouched_test_model_bundle_invalid")
...
def _expected_binding(
    ...
    *, label_evidence_required: bool,
    ...
    if label_evidence_required is not True:
        raise ValueError(
            "v22_untouched_test_authoritative_label_evidence_required"
        )
...
def _validate_reservation(
    ...
    if (
        ...
        raise ValueError("v22_untouched_test_reservation_status_invalid")
    for key, value in expected.items():
        if reservation.get(key) != value:
            raise ValueError(
                f"v22_untouched_test_reservation_binding_mismatch:{key}"
            )
    if (
        ...
        raise ValueError("v22_untouched_test_reservation_safety_invalid")
...
def _normalize_settlement_labels(
    ...
    if not isinstance(raw_labels, Mapping):
        raise ValueError("v22_untouched_test_settlement_labels_invalid")
    labels = {}
    for raw_id, value in raw_labels.items():
        try:
            row_id = int(raw_id)
        except (TypeError, ValueError) as exc:
            raise ValueError("v22_untouched_test_settlement_labels_invalid") from exc
    ...
    if tuple(sorted(labels)) != tuple(sorted(int(value) for value in expected_ids)):
        raise ValueError("v22_untouched_test_settlement_label_identity_invalid")
    return labels
...
def _validate_result(
    ...
    if not isinstance(report, Mapping) or not isinstance(settlement_rows, list) or not isinstance(survival_rows, list):
        raise ValueError("v22_untouched_test_result_invalid")
    try:
        ...
        raise ValueError("v22_untouched_test_result_invalid") from exc
...
def run_untouched_test_once(
    *, seal: Mapping[str, Any], pretest_reservation_path: Path,
    ...
    if result_path.exists():
        raise ValueError("v22_untouched_test_output_exists_without_reservation")
    if confirmation != audit_identity.UNTOUCHED_TEST_CONFIRMATION:
        raise ValueError(
            "v22_untouched_test_explicit_one_shot_confirmation_required"
        )
