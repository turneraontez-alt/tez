"""Manual authoritative-Kalshi command for V22's frozen pretest labels."""
...
from tools import q15_rti_v22_pretest_runner as runner


DEFAULT_STATE_DIR = ROOT / "reports" / "q15_rti_v22_audit"
...
def load_feature_seal(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("v22_pretest_command_feature_seal_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ValueError("v22_pretest_command_feature_seal_root_not_object")
    payload = dict(value)
...
class KalshiFeeVerifiedSQLiteLabelReader:
    ...
    def _verify_series_fees(self) -> dict[str, Any]:
        ...
        if (
            ...
            raise ValueError("v22_pretest_command_local_fee_contract_mismatch")
        records = []
        for ticker in sorted(str(value) for value in config["series_tickers"]):
            ...
            try:
                ...
                raise ValueError(
                    "v22_pretest_command_kalshi_series_unavailable"
                ) from exc
            if not isinstance(series, Mapping):
                raise ValueError("v22_pretest_command_kalshi_series_unavailable")
            ...
            try:
                ...
                raise ValueError(
                    "v22_pretest_command_kalshi_series_fee_mismatch"
                ) from exc
            if (
                returned != ticker
    ...
    def __call__(
        ...
        if self._fee_verification is None:
            raise ValueError(
                "v22_pretest_command_fee_precondition_not_verified"
            )
        labels = self.reader(row_ids)
        raw_evidence = getattr(labels, "audit_evidence", None)
        if not isinstance(raw_evidence, Mapping):
            raise ValueError("v22_pretest_command_settlement_evidence_missing")
...
def main() -> None:
    ...
    parser.add_argument("--strategy-db", default=str(feature_seal.STRATEGY_DB))
