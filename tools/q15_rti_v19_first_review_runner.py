"""Append-only one-shot runner for V19's first prospective review.

This module has no database, network, Telegram, promotion, or order capability.
It reserves the exact sealed parent/delayed population before invoking the
...
RESERVED_STATUS = "V19_FIRST_REVIEW_LABEL_ACCESS_RESERVED"
PASS_STATUS = "V19_FIRST_REVIEW_GATE_PASSED_MANUAL_CONSIDERATION_ONLY"
REJECT_STATUS = "V19_FIRST_REVIEW_GATE_FAILED_NO_PAPER_NOTIFICATIONS"
...
def _validate_sealed(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if expected != _canonical_sha256(result):
        raise ValueError("v19_first_review_state_sha256_invalid")
...
def _read_sealed(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("v19_first_review_state_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ValueError("v19_first_review_state_root_not_object")
...
def _now_iso(value: str | None) -> str:
    ...
    try:
        ...
        raise ValueError("v19_first_review_timestamp_invalid") from exc
    if parsed.tzinfo is None:
        raise ValueError("v19_first_review_timestamp_invalid")
...
def _parent_ids(rows: Sequence[Mapping[str, Any]]) -> tuple[int, ...]:
    return tuple(sorted(int(row["parent_id"]) for row in rows))


def _pair_ids(rows: Sequence[Mapping[str, Any]]) -> tuple[tuple[int, int], ...]:
    return tuple(sorted(
        (int(row["parent_id"]), int(row["delayed_id"])) for row in rows
    ))
...
def _candidate_rows(
    ...
    wanted = {(int(value[0]), int(value[1])) for value in candidate_pair_ids}
    return [
        dict(row) for row in control_rows
        if (int(row["parent_id"]), int(row["delayed_id"])) in wanted
    ]
...
def _expected_binding(
    *,
    ...
    control_ids = _parent_ids(control_rows)
    candidate = _candidate_rows(control_rows, candidate_pair_ids)
    candidate_pairs = _pair_ids(candidate)
    return {
        ...
        "parent_protocol_id": str(seal["parent_protocol_id"]),
        "parent_protocol_sha256": str(seal["parent_protocol_sha256"]),
        ...
        "candidate_pair_ids_sha256": _canonical_sha256(candidate_pairs),
...
def _validate_reservation(
    ...
    if (
        ...
        raise ValueError("v19_first_review_reservation_status_invalid")
    for key, value in expected.items():
        if reservation.get(key) != value:
            raise ValueError(f"v19_first_review_reservation_binding_mismatch:{key}")
    if any(reservation.get(key) is not False for key in (
        ...
        raise ValueError("v19_first_review_reservation_safety_invalid")
...
def _validate_result(
    ...
    if not isinstance(report, Mapping) or not isinstance(label_rows, list):
        raise ValueError("v19_first_review_result_invalid")
    try:
        pairs = sorted(
            [int(item["parent_id"]), int(item["label_yes"])]
            for item in label_rows
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError("v19_first_review_result_invalid") from exc
    ...
    if (
        ...
        != reservation.get("control_parent_ids_sha256")
        ...
        or report.get("protocol_id") != v19_identity.PROTOCOL_ID
        or report.get("protocol_sha256") != v19_identity.PROTOCOL_SHA256
        ...
        or report.get("control_parent_ids_sha256")
        != reservation.get("control_parent_ids_sha256")
        or report.get("candidate_pair_ids_sha256")
        != reservation.get("candidate_pair_ids_sha256")
...
def run_first_review_once(
    *,
    ...
    protocol = dict(protocol or v19.load_protocol())
    if contract != prospective_seal.load_contract() or protocol != v19.load_protocol():
        raise ValueError("v19_first_review_runner_contract_identity_invalid")
    controls = [dict(row) for row in control_rows]
    requested_pairs = tuple(sorted(
        (int(value[0]), int(value[1])) for value in candidate_pair_ids
    ))
    if (
        not controls
        or len(_parent_ids(controls)) != len(set(_parent_ids(controls)))
        or len(_pair_ids(controls)) != len(set(_pair_ids(controls)))
        ...
        raise ValueError("v19_first_review_runner_input_identity_invalid")
    expected = _expected_binding(
        seal=seal,
        control_rows=controls,
        candidate_pair_ids=requested_pairs,
        label_evidence_required=require_label_evidence,
    )
    if (
        expected["candidate_pair_ids_sha256"]
        != seal.get("selected_candidate_pair_ids_sha256")
        or expected["control_parent_ids_sha256"]
        ...
        raise ValueError("v19_first_review_runner_seal_binding_mismatch")
    ...
    if result_path.exists():
        raise ValueError("v19_first_review_result_exists_without_reservation")
    if confirmation != identity.CONFIRMATION_PHRASE:
        raise ValueError("v19_first_review_explicit_one_shot_confirmation_required")
    ...
    labeled = [
        {**row, "label_yes": int(labels[int(row["parent_id"])])}
        for row in controls
    ]
    ...
    result = _write_exclusive(result_path, {
        ...
        "control_label_rows": [
            {"parent_id": row_id, "label_yes": label}
            for row_id, label in pairs
        ],
