"""Crash-safe one-shot V22 TRAIN/CALIBRATION/POLICY runner.
...
from q15_upgrade.strategy_bots import rti_microstructure_v22_audit_identity as audit_identity
...
RESERVED_STATUS = "V22_TRAIN_CAL_POLICY_LABEL_ACCESS_RESERVED"
PASS_STATUS = "V22_PRETEST_GATES_PASSED_UNTOUCHED_TEST_REMAINS_SEALED"
REJECT_STATUS = "V22_PRETEST_GATE_FAILED_UNTOUCHED_TEST_REMAINS_SEALED"
...
def _validate_sealed(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if supplied != _canonical_sha256(output):
        raise ValueError("v22_pretest_state_sha256_invalid")
...
def _read_sealed(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("v22_pretest_state_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ValueError("v22_pretest_state_root_not_object")
...
def _now_iso(value: str | None) -> str:
    ...
    try:
        ...
        raise ValueError("v22_pretest_timestamp_invalid") from exc
    if parsed.tzinfo is None:
        raise ValueError("v22_pretest_timestamp_invalid")
...
def _validate_reservation(
    ...
    if (
        ...
        raise ValueError("v22_pretest_reservation_status_invalid")
    for key, value in expected.items():
        if reservation.get(key) != value:
            raise ValueError(f"v22_pretest_reservation_binding_mismatch:{key}")
    ...
    if (
        ...
        raise ValueError("v22_pretest_reservation_safety_invalid")
...
def _normalize_settlement_labels(
    ...
    if not isinstance(raw_labels, Mapping):
        raise ValueError("v22_pretest_settlement_labels_invalid")
    labels = {}
    for raw_id, value in raw_labels.items():
        try:
            row_id = int(raw_id)
        except (TypeError, ValueError) as exc:
            raise ValueError("v22_pretest_settlement_labels_invalid") from exc
    ...
    if tuple(sorted(labels)) != tuple(sorted(int(value) for value in expected_ids)):
        raise ValueError("v22_pretest_settlement_label_identity_invalid")
    return labels
...
def _survival_labels(
    ...
    for row in rows:
        ...
        if side not in {"YES", "NO"}:
            raise ValueError("v22_pretest_side_invalid")
...
def _validate_fee_evidence(evidence: Mapping[str, Any] | None) -> None:
    if not isinstance(evidence, Mapping):
        raise ValueError("v22_pretest_fee_evidence_required")
    config = modeling.load_contract()["fee_schedule_verification"]
    endpoint_template = str(config["official_endpoint"])
    expected_base_url = endpoint_template.rsplit(
        "/series/{series_ticker}", 1
    )[0]
    fee = evidence.get("fee_schedule_verification")
    if not isinstance(fee, Mapping):
        raise ValueError("v22_pretest_fee_evidence_invalid")
    ...
    for row in series:
        ...
        if (
            ...
            or str(row.get("source_url") or "")
            != endpoint_template.replace("{series_ticker}", ticker)
        ):
            raise ValueError("v22_pretest_fee_evidence_invalid")
    if (
        evidence.get("source_base_url") != expected_base_url
        or fee.get("verification_status")
...
def _write_artifact_exclusive(
    ...
    payload = {
        "artifact_version": "q15-rti-v22-historical-audit-model-bundle-v1",
        "modeling_version": audit_identity.MODELING_VERSION,
        "evaluator_contract_id": audit_identity.EVALUATOR_CONTRACT_ID,
        "evaluator_contract_sha256": audit_identity.EVALUATOR_CONTRACT_SHA256,
        "protocol_id": identity.PROTOCOL_ID,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        raise ValueError("v22_pretest_artifact_already_exists")
    temporary = None
    try:
...
def _validate_artifact(path: Path, result: Mapping[str, Any]) -> None:
    if (
        ...
        raise ValueError("v22_pretest_artifact_file_invalid")
    try:
        payload = joblib.load(path)
    except Exception as exc:
        raise ValueError("v22_pretest_artifact_unreadable") from exc
    if (
        ...
        raise ValueError("v22_pretest_artifact_identity_or_safety_invalid")
...
def _validate_result(
    ...
    if not isinstance(report, Mapping) or not isinstance(settlement_rows, list) or not isinstance(survival_rows, list):
        raise ValueError("v22_pretest_result_invalid")
    try:
        ...
        raise ValueError("v22_pretest_result_invalid") from exc
    ...
    if passed:
        ...
        raise ValueError("v22_pretest_rejected_artifact_invalid")
...
def run_pretest_once(
    *, seal: Mapping[str, Any], reservation_path: Path, confirmation: str,
    ...
    """Reserve and evaluate the exact frozen V22 non-test label population."""
    if require_label_evidence is not True:
        raise ValueError("v22_pretest_authoritative_label_evidence_required")
    feature_seal.validate_seal(seal)
    ...
    if result_path.exists() or artifact_path.exists():
        raise ValueError("v22_pretest_output_exists_without_reservation")
    if confirmation != audit_identity.PRETEST_CONFIRMATION:
        raise ValueError("v22_pretest_explicit_one_shot_confirmation_required")
