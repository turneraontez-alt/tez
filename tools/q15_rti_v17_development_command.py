"""Manual, one-shot command for V17's sealed development evaluation."""
...
from tools import q15_rti_v17_development_seal as development_seal
...
def load_seal(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("v17_development_command_seal_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ValueError("v17_development_command_seal_root_not_object")
    result = dict(value)
    development_seal.validate_seal(result)
    if result.get("seal_sha256") != identity.DEVELOPMENT_SEAL_SHA256:
        raise ValueError("v17_development_command_seal_identity_mismatch")
    return result
...
def main() -> None:
    ...
    examples = development_seal.reconstruct_examples(
