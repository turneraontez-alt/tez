"""Append-only one-shot runner for V16's sealed development audit.

This module has no database, network, Telegram, artifact, promotion, or order
capability.  A caller supplies one callback for exactly the sealed row IDs.
The exclusive reservation is durable before that callback can execute.
"""
...
RESERVED_STATUS = "V16_DEVELOPMENT_LABEL_ACCESS_RESERVED"
...
def _validate_sealed(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if expected != _canonical_sha256(result):
        raise ValueError("v16_development_state_sha256_invalid")
...
def _read_sealed(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("v16_development_state_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ValueError("v16_development_state_root_not_object")
...
def _now_iso(value: str | None) -> str:
    ...
    try:
        ...
        raise ValueError("v16_development_timestamp_invalid") from exc
    if parsed.tzinfo is None:
        raise ValueError("v16_development_timestamp_invalid")
...
def _validate_reservation(
    ...
    if (
        ...
        raise ValueError("v16_development_reservation_status_invalid")
    for key, value in expected.items():
        if reservation.get(key) != value:
            raise ValueError(f"v16_development_reservation_binding_mismatch:{key}")
    if any(reservation.get(key) is not False for key in (
        ...
        raise ValueError("v16_development_reservation_safety_invalid")
...
def _validate_result(
    ...
    if not isinstance(report, Mapping) or not isinstance(label_rows, list):
        raise ValueError("v16_development_result_invalid")
    try:
        pairs = sorted(
            [int(item["id"]), int(item["label_yes"])] for item in label_rows
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError("v16_development_result_invalid") from exc
    ...
    if (
        ...
        or report.get("evaluator_contract_id")
        != identity.EVALUATOR_CONTRACT_ID
        or report.get("evaluator_contract_sha256")
        != identity.EVALUATOR_CONTRACT_SHA256
        or report.get("protocol_id") != v16_identity.PROTOCOL_ID
        or report.get("protocol_sha256") != v16_identity.PROTOCOL_SHA256
        ...
        or report.get("candidate_market_v15_v14_identical_rows") is not True
...
def run_development_once(
    *,
    ...
    if contract != evaluator.load_contract() or protocol != evaluator.load_protocol():
        raise ValueError("v16_development_runner_contract_identity_invalid")
    if (
        ...
        raise ValueError("v16_development_runner_input_identity_invalid")
    ...
    if (
        ...
        raise ValueError("v16_development_runner_seal_binding_mismatch")
    ...
    if result_path.exists():
        raise ValueError("v16_development_result_exists_without_reservation")
    if confirmation != identity.CONFIRMATION_PHRASE:
        raise ValueError("v16_development_explicit_one_shot_confirmation_required")
    ...
    try:
        ...
        raise ValueError("v16_development_labels_invalid") from exc
    if set(labels) != set(ids) or any(value not in {0, 1} for value in labels.values()):
        raise ValueError("v16_development_labels_invalid")
