"""Frozen V21 train/calibration/policy modeling with no data access.
...
def load_contract(path: Path | None = None) -> dict[str, Any]:
    contract = v21.load_evaluator_contract(
        v21.DEFAULT_EVALUATOR_CONTRACT if path is None else path
    )
    dependency = dict(contract["dependency_contract"])
    if sklearn.__version__ != dependency["exact_version"]:
        raise ValueError("v21_evaluator_scikit_learn_version_mismatch")
    return contract


def _candidate_specs(cohort: str, contract: Mapping[str, Any]) -> list[dict[str, Any]]:
    if cohort not in COHORTS:
        raise ValueError("v21_model_cohort_invalid")
    ...
    for group in groups:
        ...
        if family == "ELASTIC_NET_LOGISTIC":
            ...
            raise ValueError("v21_model_family_invalid")
    expected = 28 if cohort == "NON_BTC_TRANSFER" else 4
    if len(output) != expected:
        raise ValueError("v21_model_candidate_grid_invalid")
    return output
...
def _matrix(
    ...
    try:
        ...
        raise ValueError("v21_model_feature_matrix_invalid") from exc
    if (
        matrix.ndim != 2
        or matrix.shape != (len(rows), identity.FEATURE_COUNT)
        or not np.isfinite(matrix).all()
    ):
        raise ValueError("v21_model_feature_matrix_invalid")
    if feature_indices is None:
        return matrix
    try:
        indices = tuple(int(value) for value in feature_indices)
    except (TypeError, ValueError) as exc:
        raise ValueError("v21_model_feature_indices_invalid") from exc
    if (
        not indices
        ...
        raise ValueError("v21_model_feature_indices_invalid")
...
def _labels(rows: Sequence[Mapping[str, Any]]) -> np.ndarray:
    ...
    for row in rows:
        ...
        if isinstance(value, bool) or value not in (0, 1):
            raise ValueError("v21_model_label_invalid")
...
def _fit_scaler(matrix: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    if matrix.ndim != 2 or matrix.shape[1] <= 0:
        raise ValueError("v21_model_matrix_geometry_invalid")
    if not np.isfinite(matrix).all():
        raise ValueError("v21_model_matrix_nonfinite")
...
def _family_contract(spec: Mapping[str, Any], contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not matches:
        raise ValueError("v21_model_family_contract_missing")
    return matches[0]
...
def _fit(
    ...
    *, feature_indices: Sequence[int] | None = None,
    ...
    if len(set(labels.tolist())) != 2:
        raise ValueError("v21_model_training_single_class")
    ...
    if any(issubclass(item.category, ConvergenceWarning) for item in caught):
        raise ValueError("v21_model_fit_did_not_converge")
    return {
        "spec": dict(spec),
...
def _predict(
    ...
    if not indices:
        raise ValueError("v21_model_fitted_feature_indices_missing")
    matrix = _matrix(rows, indices)
...
def _proper_scores(labels: np.ndarray, probabilities: np.ndarray) -> dict[str, float]:
    if len(labels) == 0 or len(labels) != len(probabilities):
        raise ValueError("v21_model_score_geometry_invalid")
    return {
...
def _cluster_proper_scores(
    ...
    if len(rows) == 0 or len(rows) != len(labels) or len(rows) != len(probabilities):
        raise ValueError("v21_model_cluster_score_geometry_invalid")
...
def _market_probabilities(
    ...
    index = v21_features.FEATURE_NAMES.index(MARKET_FEATURE)
...
def _candidate_walk_forward(
    ...
    *, feature_indices: Sequence[int] | None = None,
    ...
    if len(train_closes) != identity.TRAIN_CLOSE_WINDOWS:
        raise ValueError("v21_model_train_window_geometry_invalid")
    pooled_exec_labels = []
    ...
    for fold_index, fold in enumerate(contract["internal_walk_forward_folds"]):
        ...
        if (
            ...
            raise ValueError("v21_model_fold_chronology_invalid")
        ...
        if len(executable_rows) < minimum:
            raise ValueError("v21_model_executable_validation_minimum_not_met")
        fitted = _fit(
...
def _fit_platt(
    ...
    if len(set(labels.tolist())) != 2:
        raise ValueError("v21_platt_calibration_single_class")
    ...
    if any(issubclass(item.category, ConvergenceWarning) for item in caught):
        raise ValueError("v21_platt_fit_did_not_converge")
    return calibrator
...
def _calibrated_predict(
    ...
    if isinstance(calibrator, Mapping):
        if dict(calibrator) != {"method": "IDENTITY"}:
            raise ValueError("v21_calibrator_identity_invalid")
...
def _bootstrap_lower_mean_pnl(
    ...
    if not values:
        raise ValueError("v21_bootstrap_no_picks")
...
def _margin_report(
    ...
    for row, probability in zip(rows, probabilities, strict=True):
        if row.get("execution_supported") is not True or row.get("sim_contracts") != 10.0:
            raise ValueError("v21_policy_fake_or_partial_fill_forbidden")
        execution = rti_simulated_execution(row["entry_ask_cents"], 10, 2.0)
        if execution is None:
            raise ValueError("v21_policy_execution_evidence_invalid")
...
def _labeled_pretest_rows(
    ...
    for raw_id, value in labels.items():
        try:
            ...
            raise ValueError("v21_pretest_label_identity_invalid") from exc
        if isinstance(value, bool) or value not in (0, 1):
            raise ValueError("v21_pretest_label_invalid")
        normalized[row_id] = int(value)
    if set(normalized) != expected_ids or len(rows) != 155 * 7:
        raise ValueError("v21_pretest_label_identity_invalid")
    return [
        (
...
def evaluate_cohort(
    ...
    if any(
        ...
        raise ValueError("v21_model_cohort_geometry_invalid")
    candidate_reports = []
    ...
    ablation_config = dict(contract["v20_feature_map_ablation"])
    ...
    if len(ablation_indices) != int(ablation_config["feature_count"]):
        raise ValueError("v21_v20_ablation_feature_geometry_invalid")
    ...
    if not valid_ablation:
        return {
            "report": {
                "cohort": cohort,
                "status": "V20_FEATURE_MAP_ABLATION_WALK_FORWARD_SELECTION_FAILED",
                "candidate_reports": candidate_reports,
                "selected_model_id": selected["model_id"],
                "selected_model": selected,
                "internal_walk_forward_gate_met": True,
                ...
                "v20_feature_map_ablation_available": False,
                "probability_calibration_gate_met": False,
                "execution_policy_gate_met": False,
                "pretest_gate_met": False,
            },
    ...
    try:
        ...
        return {
            "report": {
                "cohort": cohort,
                "status": "V20_FEATURE_MAP_ABLATION_FAILED",
                "failure": str(exc),
                "candidate_reports": candidate_reports,
                "selected_model_id": selected["model_id"],
                "selected_model": selected,
    ...
    if not policy_exec:
        return {
            "report": {
                ...
                "probability_calibration_gate_met": False,
                "v20_feature_map_ablation_available": True,
                "v20_feature_map_ablation_calibration_scores": (
    ...
    report = {
        ...
        "v20_feature_map_ablation_available": True,
        "v20_feature_map_ablation_feature_count": len(ablation_indices),
        "v20_feature_map_ablation_feature_indices_sha256": _canonical_sha256(
            ablation_indices
        ),
        "v20_feature_map_ablation_candidate_reports": (
            ablation_candidate_reports
        ...
        "v20_feature_map_ablation_selected_model": ablation_selected,
        "v20_feature_map_ablation_selected_independently": True,
        "v20_feature_map_ablation_calibration_scores": (
            ablation_calibration_scores
        ),
        ...
        "v20_feature_map_ablation_policy_calibrator_candidate_scores": (
            ablation_selection["candidate_scores"]
        ),
        "v20_feature_map_ablation_used_for_v21_model_selection_calibration_or_policy_selection": False,
        ...
        "artifact": (
            {
                ...
                "v20_feature_map_ablation_base_model": ablation_fitted,
                "v20_feature_map_ablation_selected_spec": dict(
...
def evaluate_pretest(
    ...
    *, contract: Mapping[str, Any] | None = None,
    ...
    if contract != frozen:
        raise ValueError("v21_evaluator_contract_override_forbidden")
    ...
    report = {
        ...
        "status": (
            "V21_PRETEST_GATES_PASSED_UNTOUCHED_TEST_MAY_BE_MANUALLY_OPENED_ONCE"
            if gate_met else "V21_PRETEST_GATE_FAILED_UNTOUCHED_TEST_REMAINS_SEALED"
        ),
...
def _accuracy_only_metrics(
    ...
    return {
        ...
        "trade_frequency_per_complete_window": count / complete_windows,
        "pnl_reported": False,
        "pnl_not_reported_reason": "FROZEN_V18_DID_NOT_REQUIRE_TEN_CONTRACT_DEPTH",
        "parent_ids_sha256": _canonical_sha256(tuple(sorted(
...
def _trade_record(
    ...
    if row.get("execution_supported") is not True or row.get("sim_contracts") != 10.0:
        raise ValueError("v21_untouched_test_fake_or_partial_fill_forbidden")
    execution = rti_simulated_execution(row["entry_ask_cents"], 10, 2.0)
    pnl = rti_simulated_net_pnl_cents(
        row["entry_ask_cents"], bool(int(row["label_survives"])), 10, 2.0,
    )
    if execution is None or pnl is None:
        raise ValueError("v21_untouched_test_execution_invalid")
    return {
        **dict(row),
        "probability": float(probability),
...
def _tier(value: float, tiers: Sequence[Mapping[str, Any]]) -> str:
    ...
    raise ValueError("v21_untouched_test_subgroup_tier_invalid")
...
def _subgroup_values(
    ...
    feature_map = dict(zip(
        v21_features.FEATURE_NAMES, row["features"], strict=True,
    ))
    ...
    return {
        ...
        "VOLATILITY_TIER": _tier(
            math.expm1(max(0.0, float(
                feature_map["log1p_spot_fast_volatility_60s_bps"]
            ))),
            config["volatility_raw_bps_tiers"],
        ),
        "MARKET_REGIME": regime,
        "REVERSAL_RISK": _tier(
            float(probability), config["reversal_risk_probability_tiers"],
        ),
        "SETTLEMENT_AVERAGE_RISK": _tier(
            abs(float(feature_map["parent_distance_to_remaining_volatility"])),
            config["settlement_average_risk_absolute_ratio_tiers"],
        ),
        "TRAJECTORY_CURVATURE_TIER": _tier(
            float(feature_map["continuation_curvature_bps"]),
            config["trajectory_curvature_bps_tiers"],
        ),
    }
...
def _score_untouched_cohort(
    ...
    ablation_model = artifact.get("v20_feature_map_ablation_base_model")
    if (
        ...
        or artifact.get("v20_feature_map_ablation_selected_calibrator_method")
        not in {"IDENTITY", "L2_REGULARIZED_PLATT_ON_LOGIT"}
        ...
        or artifact.get("v20_feature_map_ablation_selected_model_id")
        != ablation_model.get("model_id")
        or dict(artifact.get("v20_feature_map_ablation_selected_spec") or {})
        != dict(ablation_model.get("spec") or {})
        or tuple(ablation_model.get("feature_indices") or ())
        != tuple(range(int(dict(contract["v20_feature_map_ablation"])[
            "feature_count"
        ])))
        or (
        ...
        or (
            artifact.get("v20_feature_map_ablation_selected_calibrator_method")
            == "IDENTITY"
        ) != isinstance(
            artifact.get("v20_feature_map_ablation_platt_calibrator"), Mapping
        )
    ):
        raise ValueError("v21_untouched_test_cohort_geometry_invalid")
    ...
    ablation_probabilities = _calibrated_predict(
        artifact["v20_feature_map_ablation_platt_calibrator"],
        _predict(
            artifact["v20_feature_map_ablation_base_model"], cohort_rows, contract,
        ),
        contract,
    )
    ...
    for row, probability in zip(cohort_rows, probabilities, strict=True):
        ...
        if row.get("matched_v19_eligible") is True:
            matched_v19_records.append(record)
    matched_v18_rows = [
        row for row in cohort_rows if row.get("matched_v18_eligible") is True
    ]
    ...
    return {
        ...
        "model_probability_metrics": {
            **model_scores,
            ...
            "v20_feature_map_ablation_log_loss": ablation_scores["log_loss"],
            "v20_feature_map_ablation_brier_score": ablation_scores["brier_score"],
            "selected_calibrator_method": str(
                artifact["selected_calibrator_method"]
            ),
            "v20_feature_map_ablation_selected_calibrator_method": str(
                artifact["v20_feature_map_ablation_selected_calibrator_method"]
            ),
        ...
        "matched_v18_selection": {
            "metrics": _accuracy_only_metrics(
                matched_v18_rows, complete_windows, contract,
            ),
        },
        "matched_v19_selection": {
            "metrics": _trade_metrics(
                matched_v19_records, complete_windows, contract,
            ),
        },
        ...
        "gate_checks": checks,
        "gate_met": all(checks.values()),
        "v20_feature_map_ablation_used_for_v21_gates": True,
...
def evaluate_untouched_test(
    ...
    contract = dict(frozen if contract is None else contract)
    if contract != frozen:
        raise ValueError("v21_untouched_test_contract_override_forbidden")
    feature_seal.validate_seal(seal)
    test_rows = [
    ...
    try:
        supplied_ids = {int(key) for key in survival_labels}
    except (TypeError, ValueError) as exc:
        raise ValueError("v21_untouched_test_label_identity_invalid") from exc
    if (
        ...
        raise ValueError("v21_untouched_test_input_identity_invalid")
    labeled = []
    for row in test_rows:
        value = survival_labels[int(row["parent_id"])]
        if isinstance(value, bool) or value not in (0, 1):
            raise ValueError("v21_untouched_test_label_invalid")
    ...
    label_pairs = tuple(sorted(
        (int(row_id), int(value)) for row_id, value in survival_labels.items()
    ))
    return {
        "modeling_version": MODELING_VERSION,
        "evaluator_contract_id": identity.EVALUATOR_CONTRACT_ID,
        ...
        "status": (
            "V21_HISTORICAL_GATES_PASSED_MANUAL_PAPER_CONSIDERATION_ONLY"
            if passed else "V21_UNTOUCHED_TEST_GATE_FAILED_NO_PAPER_CHALLENGER"
        ),
