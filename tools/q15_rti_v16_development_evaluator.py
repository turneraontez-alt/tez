"""Pure in-memory V16 development walk-forward evaluator.
...
from q15_upgrade.strategy_bots import rti_microstructure_v16_audit_identity as audit_identity
...
DEFAULT_PROTOCOL = ROOT / v16_identity.PROTOCOL_RELATIVE_PATH
...
def load_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    contract = _load(path, "v16_evaluator_contract_unreadable")
    ...
    if (
        ...
        != "FROZEN_BEFORE_ANY_V16_DEVELOPMENT_LABEL_ACCESS"
        or contract.get("protocol_id") != v16_identity.PROTOCOL_ID
        or contract.get("protocol_sha256") != v16_identity.PROTOCOL_SHA256
        ...
        or list(trust.get("fixed_factor_grid") or ())
        ...
        raise ValueError("v16_evaluator_contract_identity_or_safety_invalid")
    return contract


def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    protocol = _load(path, "v16_evaluator_protocol_unreadable")
    if (
        design_fingerprint(protocol) != v16_identity.PROTOCOL_SHA256
        or protocol.get("protocol_id") != v16_identity.PROTOCOL_ID
        or protocol.get("design_id") != v16_identity.DESIGN_ID
    ):
        raise ValueError("v16_evaluator_protocol_identity_invalid")
    return protocol


def _architecture_configs(
    protocol: Mapping[str, Any],
) -> dict[str, dict[str, Any]]:
    v15_design = _load(DEFAULT_V15_DESIGN, "v16_evaluator_v15_design_unreadable")
    v14_design = _load(DEFAULT_V14_DESIGN, "v16_evaluator_v14_design_unreadable")
    if (
        design_fingerprint(v15_design) != v15_identity.DESIGN_SHA256
        ...
        raise ValueError("v16_evaluator_comparator_design_identity_invalid")
    return {
        "V16": dict(protocol["model"]),
...
def _validate_examples(
    ...
    if len(examples) != 1440 or len({int(row["id"]) for row in examples}) != 1440:
        raise ValueError("v16_evaluator_row_geometry_invalid")
    windows = tuple(sorted({float(row["close_time"]) for row in examples}))
    if len(windows) != 240:
        raise ValueError("v16_evaluator_window_geometry_invalid")
    for close_time in windows:
        ...
        if len(rows) != 6 or assets != COHORT_ASSETS:
            raise ValueError("v16_evaluator_same_close_asset_leakage")
        for row in rows:
            candidate = [float(value) for value in row.get("v16_features") or ()]
            control15 = [float(value) for value in row.get("v15_features") or ()]
            control14 = [float(value) for value in row.get("v14_features") or ()]
            if (
                tuple(row.get("v16_feature_names") or ()) != v16.FEATURE_NAMES
                or tuple(row.get("v15_feature_names") or ()) != v15.FEATURE_NAMES
                or tuple(row.get("v14_feature_names") or ()) != v14.FEATURE_NAMES
                or len(candidate) != 45
                or len(control15) != 25
                or len(control14) != 20
                or candidate[:25] != control15
                or control15[:20] != control14
                or int(row.get("label_yes", -1)) not in {0, 1}
                or not 0.0 < float(row["market_yes_probability"]) < 1.0
                or str(row.get("asset") or "").upper() == "BTC"
            ):
                raise ValueError("v16_evaluator_example_invalid")
    return windows
...
def _with_features(
    ...
    names_key = {
        "v16_features": "v16_feature_names",
        "v15_features": "v15_feature_names",
        "v14_features": "v14_feature_names",
    }.get(key)
    if names_key is None:
        raise ValueError("v16_evaluator_feature_key_invalid")
    return [{
        **dict(row),
...
def _trust_protocol(contract: Mapping[str, Any]) -> dict[str, Any]:
    trust = dict(contract["trust_selection"])
    return {
        "residual_trust_selection": {
            "architecture": "v16_preregistered_inner_oof_trust_v1",
            "fixed_factor_grid": list(trust["fixed_factor_grid"]),
            ...
            "bootstrap": {
                ...
                "random_seed": int(trust["bootstrap_random_seed"]),
...
def _predict(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("v16_evaluator_trust_label_leakage")
...
def _losses(
    ...
    if not rows or len(rows) != len(probabilities):
        raise ValueError("v16_evaluator_loss_geometry_invalid")
    labels = np.asarray([int(row["label_yes"]) for row in rows], dtype=float)
    values = np.clip(np.asarray(probabilities, dtype=float), 1e-6, 1.0 - 1e-6)
    if not np.isin(labels, (0.0, 1.0)).all() or not np.isfinite(values).all():
        raise ValueError("v16_evaluator_loss_evidence_invalid")
    return (
        np.square(values - labels),
        -(labels * np.log(values) + (1.0 - labels) * np.log(1.0 - values)),
    )


def _wilson(correct: int, count: int) -> tuple[float, float]:
    z = 1.959963984540054
    rate = correct / count
...
def proper_scores(
    ...
    correct = sum(
        (float(probability) >= 0.5) == bool(int(row["label_yes"]))
        for row, probability in zip(rows, probabilities)
    )
    low, high = _wilson(correct, len(rows))
    return {
        "rows": len(rows),
        "close_windows": len({float(row["close_time"]) for row in rows}),
        "correct": correct,
...
def paired_comparator_bootstrap(
    ...
    *,
    ...
    candidate_brier, candidate_log = _losses(rows, candidate)
    comparator_brier, comparator_log = _losses(rows, comparator)
    ...
    if (
        ...
        raise ValueError("v16_evaluator_bootstrap_contract_invalid")
    ...
    return {
        "version": "q15-rti-v16-paired-close-window-bootstrap-v1",
        "cluster_key": "close_time",
        ...
        "same_close_assets_resampled_together": True,
        "loss_delta_direction": f"V16_MINUS_{comparator_name}",
...
def _outer_folds(windows: Sequence[float]) -> list[dict[str, Any]]:
    ...
    for index in range(4):
        ...
        if len(validation) != 30 or max(train) >= min(validation):
            raise ValueError("v16_evaluator_outer_chronology_invalid")
...
def evaluate_development(
    ...
    *,
    ...
    # Revalidate supplied mappings, not merely their default files.
    if (
        design_fingerprint(contract) != audit_identity.EVALUATOR_CONTRACT_SHA256
        or design_fingerprint(protocol) != v16_identity.PROTOCOL_SHA256
    ):
        raise ValueError("v16_evaluator_supplied_contract_identity_invalid")
    windows = _validate_examples(labeled_development)
    configs = _architecture_configs(protocol)
    trust_protocol = _trust_protocol(contract)
    comparison = dict(contract["aggregate_comparison"])
    seeds = {
    ...
    aggregate = {name: [] for name in ("V16", "V15", "V14", "MARKET")}
    folds = []
    for fold in _outer_folds(windows):
        train_times = set(fold["train"])
        validation_times = set(fold["validation"])
        train = [row for row in labeled_development if float(row["close_time"]) in train_times]
        validation = [row for row in labeled_development if float(row["close_time"]) in validation_times]
        ...
        folds.append({
            ...
            "selected_residual_trust_factor": {
                ...
                for name in ("V16", "V15", "V14")
            },
            "out_of_distribution_rows": ood,
            "scores": {
                name: proper_scores(validation, predictions[name])
                for name in ("V16", "V15", "V14", "MARKET")
            },
            ...
            "candidate_not_worse_than_v15": bool(
                comparisons["V15"]["candidate_minus_comparator_brier"] <= 0.0
                and comparisons["V15"]["candidate_minus_comparator_log_loss"] <= 0.0
            ),
    ...
    checks.update({
        ...
        "every_fold_not_worse_vs_v15": all(
            bool(fold["candidate_not_worse_than_v15"]) for fold in folds
        ),
        ...
        "v15_brier_mean_effect_floor": bool(
            aggregate_comparisons["V15"]["candidate_minus_comparator_brier"]
            <= float(comparison["candidate_minus_v15_brier_mean_must_be_at_most"])
        ),
        "v15_log_loss_mean_effect_floor": bool(
            aggregate_comparisons["V15"]["candidate_minus_comparator_log_loss"]
            <= float(comparison["candidate_minus_v15_log_loss_mean_must_be_at_most"])
        ),
    })
    ...
    return {
        ...
        "protocol_id": v16_identity.PROTOCOL_ID,
        "protocol_sha256": v16_identity.PROTOCOL_SHA256,
        ...
        "candidate_market_v15_v14_identical_rows": True,
        ...
        "aggregate": {
            "scores": {
                name: proper_scores(aggregate_rows, aggregate[name])
                for name in ("V16", "V15", "V14", "MARKET")
            },
        ...
        "gate_checks": checks,
        "gate_met": passed,
        "failure_result": None if passed else "NO_V16_CALIBRATION_LABEL_ACCESS",
