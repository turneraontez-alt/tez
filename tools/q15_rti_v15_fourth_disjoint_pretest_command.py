"""Run V15's fourth fully disjoint NON-BTC pretest once."""
from __future__ import annotations

import argparse
import json
from pathlib import Path
import sys

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from tools import q15_rti_v15_audit_seal as audit_seal
from tools import q15_rti_v15_fourth_disjoint_audit as fourth
from tools import q15_rti_v15_pretest as pretest
from tools import q15_rti_v15_pretest_command as command
from tools.q15_rti_microstructure_freeze import load_feature_rows


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--seal", default=str(fourth.DEFAULT_OUTPUT))
    parser.add_argument("--strategy-db", default=str(audit_seal.DEFAULT_DB))
    parser.add_argument("--reservation", default=str(fourth.DEFAULT_PRETEST_STATE))
    parser.add_argument("--confirmation", required=True)
    args = parser.parse_args()
    seal = fourth.load_seal(Path(args.seal))
    database = Path(args.strategy_db)
    selected = fourth.select_rows(load_feature_rows(database), seal)
    design, _v14, _charter, protocol, _geometry, _sha = (
        audit_seal._load_inputs()
    )
    result = pretest.run_pretest_once(
        seal=seal,
        selected_feature_rows=selected,
        design=design,
        protocol=protocol,
        cohort=fourth.COHORT,
        reservation_path=Path(args.reservation),
        confirmation=args.confirmation,
        read_pretest_labels=command.KalshiVerifiedSQLiteLabelReader(
            database, expected_rows=selected,
        ),
        require_label_evidence=True,
    )
    print(json.dumps(result, indent=2, sort_keys=True, allow_nan=False))


if __name__ == "__main__":
    main()
