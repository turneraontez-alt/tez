...
from tools import q15_rti_v21_pretest_command as command
from test_q15_rti_v21_modeling import _population


def test_v21_command_loads_exact_seal_and_projects_contracts_without_outcomes(
    tmp_path,
):
...
def test_v21_fee_reader_verifies_every_frozen_series(tmp_path):
    seal, _labels = _population()
    ...
    reader = command.KalshiFeeVerifiedSQLiteLabelReader(
        ...
        get_market=lambda _ticker: None,
...
def test_v21_fee_reader_fails_closed_on_one_changed_series(tmp_path):
    seal, _labels = _population()
