...
from tools import q15_rti_v19_first_review_command as command
...
def _rows() -> list[dict]:
    return [{
        ...
        "parent_entry_ask_cents": 50.0,
        "delayed_entry_ask_cents": 45.0,
...
def _candidate_pairs(rows: list[dict]):
    return tuple(
        (int(row["parent_id"]), int(row["delayed_id"])) for row in rows[:30]
    )


def _seal(rows: list[dict]) -> dict:
    candidate = [dict(row) for row in rows[:30]]
    pairs = _candidate_pairs(rows)
    control_ids = tuple(range(1, 61))
    return {
        "seal_sha256": "synthetic-seal",
        "parent_protocol_id": "parent-protocol",
        "parent_protocol_sha256": "parent-sha",
        "selected_complete_close_windows": 150,
        "selected_candidate_picks": 30,
        "selected_control_picks": 60,
        "selected_candidate_pair_ids_sha256": runner._canonical_sha256(pairs),
        "selected_control_parent_ids_sha256": runner._canonical_sha256(control_ids),
        "selected_candidate_feature_evidence_sha256": runner._canonical_sha256(candidate),
...
def _fake_report(rows: list[dict], gate: bool = True) -> dict:
    return {
        ...
        "protocol_id": runner.v19_identity.PROTOCOL_ID,
        "protocol_sha256": runner.v19_identity.PROTOCOL_SHA256,
        ...
        "selected_complete_close_windows": 150,
        "gate_met": gate,
        "control_parent_ids_sha256": runner._canonical_sha256(tuple(range(1, 61))),
        "candidate_pair_ids_sha256": runner._canonical_sha256(_candidate_pairs(rows)),
...
def test_wrong_confirmation_never_reserves_or_calls_labels(tmp_path):
    ...
    with pytest.raises(ValueError, match="explicit_one_shot_confirmation"):
        runner.run_first_review_once(
            seal=_seal(rows), control_rows=rows,
            candidate_pair_ids=_candidate_pairs(rows),
...
def test_reservation_precedes_callback_and_result_is_idempotent(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda labeled, candidate_pair_ids, seal, contract: _fake_report(rows),
    )
    first = runner.run_first_review_once(
        seal=_seal(rows), control_rows=rows,
        candidate_pair_ids=_candidate_pairs(rows),
        reservation_path=reservation, confirmation=identity.CONFIRMATION_PHRASE,
        read_control_labels=read, timestamp="2026-08-04T00:00:00Z",
    )
...
def test_tampered_contract_fails_before_reservation_or_labels(tmp_path):
    ...
    with pytest.raises(ValueError, match="contract_identity"):
        runner.run_first_review_once(
            ...
            reservation_path=reservation,
            confirmation=identity.CONFIRMATION_PHRASE,
            read_control_labels=lambda _ids: {}, contract=contract,
        )
    assert reservation.exists() is False
...
def test_feature_tampering_fails_before_reservation_or_labels(tmp_path):
    rows = _rows()
    artifact = _seal(rows)
    rows[0]["delayed_entry_ask_cents"] = 46.0
    reservation = tmp_path / "reservation.json"
    with pytest.raises(ValueError, match="seal_binding"):
        runner.run_first_review_once(
            seal=artifact, control_rows=rows,
            candidate_pair_ids=_candidate_pairs(rows),
            reservation_path=reservation,
            confirmation=identity.CONFIRMATION_PHRASE,
            read_control_labels=lambda _ids: {},
        )
    assert reservation.exists() is False
...
def test_command_rejects_unbound_seal(tmp_path, monkeypatch):
    ...
    path.write_text('{"seal_sha256":"tampered"}', encoding="utf-8")
