...
from tools import q15_rti_v15_paper_preregister as preregister
from tools.q15_rti_microstructure_preregister import design_fingerprint


def test_frozen_v15_paper_protocol_validates_outcome_blind():
...
@pytest.mark.parametrize(
    ("section", "key", "value"),
    (
        ("activation_prerequisites", "historical_results_can_promote", True),
        (
        ...
        ("evidence_and_timestamp_gate", "reused_quote_forbidden", False),
        (
            "probability_and_entry_policy",
...
def test_any_frozen_paper_protocol_tamper_fails(
    section, key, value,
):
...
def test_v15_paper_protocol_freezes_exact_ledger_and_review_contract():
    protocol = preregister.load_protocol()
    ...
    assert ledger["duplicate_decision_returns_existing_row_without_rescore"]
...
def test_v15_paper_preregister_has_no_runtime_or_outcome_capability():
