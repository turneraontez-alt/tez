...
def test_clustered_bootstrap_is_deterministic():
    ...
    assert first["same_close_assets_resampled_together"] is True
...
def test_walk_forward_uses_identical_rows_and_never_future_partitions(monkeypatch):
    ...
    def fake_predict(train, validation, *, feature_key, config, trust_protocol):
        probability = {
            "v16_features": (0.80, 0.20),
    ...
    assert result["candidate_market_v15_v14_identical_rows"] is True
