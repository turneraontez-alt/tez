...
from tools import q15_rti_v16_development_command as command
...
def _fake_report(rows: list[dict], gate: bool = True) -> dict:
    return {
        ...
        "protocol_id": runner.v16_identity.PROTOCOL_ID,
        "protocol_sha256": runner.v16_identity.PROTOCOL_SHA256,
...
def test_reservation_precedes_callback_and_result_is_idempotent(
    ...
    assert calls == 1
    assert runner.result_path_for(reservation).exists()
...
def test_command_loads_only_the_bound_development_seal(tmp_path):
    real = command.load_seal(
        command.ROOT / identity.DEVELOPMENT_SEAL_RELATIVE_PATH
    )
    tampered = dict(real)
