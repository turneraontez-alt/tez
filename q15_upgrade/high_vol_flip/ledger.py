...
_WATCH_SCHEMA = """
CREATE TABLE IF NOT EXISTS hvf_watch (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    created_at REAL NOT NULL,
    model_version TEXT NOT NULL,
    record_kind TEXT NOT NULL DEFAULT 'EARLY_FLIP_WATCH',
    asset TEXT NOT NULL,
    ticker TEXT NOT NULL,
    interval TEXT NOT NULL,
    window_key INTEGER NOT NULL,
    ...
    correct INTEGER,
    hypothetical_pnl_cents REAL,
    UNIQUE(model_version, ticker, window_key, interval, rule_code)
);
CREATE INDEX IF NOT EXISTS idx_hvf_watch_resolve
    ON hvf_watch(model_version, official_result, close_time);
CREATE INDEX IF NOT EXISTS idx_hvf_watch_asset_rule
    ON hvf_watch(model_version, asset, rule_code);
"""
...
class HighVolFlipLedger:
    _COLS = (
        ...
        "no_ask_depth_contracts", "selected_depth_ratio", "kalshi_depth_status",
    ...
    def __init__(self, db_path: str) -> None:
        ...
        with self._lock:
            ...
            self._conn.executescript(_WATCH_SCHEMA)
    ...
    def record_watch(self, row: Mapping[str, Any]) -> int | None:
        values = []
        for col in self._WATCH_COLS:
            if col == "record_kind":
                values.append(row.get(col) or "EARLY_FLIP_WATCH")
            elif col == "paper_only":
                values.append(1 if row.get(col, True) else 0)
            elif col == "delivery_status":
                values.append(row.get(col) or "RECORDED")
            else:
                values.append(row.get(col))
        placeholders = ",".join("?" for _ in self._WATCH_COLS)
        with self._lock:
            try:
                cur = self._conn.execute(
                    f"INSERT INTO hvf_watch({','.join(self._WATCH_COLS)}) VALUES({placeholders})",
                    values,
                )
                self._conn.commit()
                return int(cur.lastrowid)
    ...
    def unresolved_closed(self, model_version: str, now: float) -> list[str]:
        with self._lock:
            rows = self._conn.execute(
                ...
                "SELECT ticker FROM hvf_alerts "
                "WHERE model_version=? AND official_result IS NULL "
                "AND close_time IS NOT NULL AND close_time <= ? "
                "UNION "
                "SELECT ticker FROM hvf_watch "
                "WHERE model_version=? AND official_result IS NULL "
                "AND close_time IS NOT NULL AND close_time <= ?"
                ") LIMIT 500",
                (model_version, now, model_version, now),
            ).fetchall()
    ...
    def resolve(self, model_version: str, ticker: str, official_result: str,
        ...
        with self._lock:
            graded = self._resolve_table_locked(
                "hvf_alerts", model_version, ticker, official, ts
            )
            graded += self._resolve_table_locked(
                "hvf_watch", model_version, ticker, official, ts
            )
            self._conn.commit()
        return graded

    def _resolve_table_locked(self, table: str, model_version: str, ticker: str,
                              official: str, ts: float) -> int:
        rows = self._conn.execute(
            f"SELECT id, predicted_outcome, entry_ask_cents FROM {table} "
        ...
        for r in rows:
            ...
            self._conn.execute(
                f"UPDATE {table} SET official_result=?, resolved_at=?, "
    ...
    def watch_rows(self, model_version: str) -> list[dict[str, Any]]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT * FROM hvf_watch WHERE model_version=? ORDER BY id",
                (model_version,),
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def scoreboard(self, model_version: str) -> dict[str, Any]:
        with self._lock:
            ...
            watch_rows = self._conn.execute(
                "SELECT * FROM hvf_watch WHERE model_version=?",
                (model_version,),
            ).fetchall()
        ...
        return {
            ...
            **self._score_sections(all_rows),
            "early_watch": self._watch_scoreboard(model_version, list(watch_rows)),
        }

    def _watch_scoreboard(self, model_version: str,
                          rows: Sequence[sqlite3.Row]) -> dict[str, Any]:
        all_rows = list(rows)
        return {
            "available": True,
            "model_version": model_version,
            "record_kind": "EARLY_FLIP_WATCH",
    ...
    @classmethod
    def _score_sections(cls, rows: Sequence[sqlite3.Row]) -> dict[str, Any]:
        all_rows = list(rows)
        ...
        return {
            "overall": cls._agg(all_rows),
            "by_asset": {
                asset: cls._agg([r for r in all_rows if str(r["asset"] or "") == asset])
                for asset in by_asset
            },
            "by_rule": {
                rule: cls._agg([r for r in all_rows if str(r["rule_code"] or "") == rule])
                for rule in by_rule
            },
            "by_asset_rule": {
                f"{asset}:{rule}": cls._agg([
                    r for r in all_rows
